...
contract DelegateFixedQuorum is DelegateEvents {
    ...
    constructor(
        address _vetoer,
        address _nounishToken,
        NounsDAOStorageV1 _ownerDAO,
        uint256 _execWindow,
        uint256 _quorum
    ) {
        require(address(_ownerDAO) != address(0), "invalid owner dao address");

        require(_nounishToken != address(0), "invalid nounish NFT address");
    ...
    function execute(uint256 proposalId) external {
        ...
        INounsDAOGovernanceV2 eDAO = INounsDAOGovernanceV2(proposal.eDAO);
        if (r == ProposalResult.For) {
            eDAO.castRefundableVote(proposal.eID, 1);
        } else if (r == ProposalResult.Against) {
            eDAO.castRefundableVote(proposal.eID, 0);
        } else if (r == ProposalResult.Abstain) {
            eDAO.castRefundableVote(proposal.eID, 2);
        }

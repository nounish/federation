...
import {MetadataBuilder} from "micro-onchain-metadata-utils/src/MetadataBuilder.sol";
import {MetadataJSONKeys} from "micro-onchain-metadata-utils/src/MetadataJSONKeys.sol";
    ...
    MetadataRendererStorageV2
{
    ...
    function setAdditionalTokenProperties(AdditionalTokenProperty[] memory _additionalTokenProperties)
        external
        onlyOwner
    {
    ...
    function _addProperties(string[] calldata _names, ItemParam[] calldata _items, IPFSGroup calldata _ipfsGroup)
        internal
    {
    ...
    function _getItemImage(Item memory _item, string memory _propertyName) private view returns (string memory) {
        return UriEncode.uriEncode(
            string(
                abi.encodePacked(
                    ipfsData[_item.referenceSlot].baseUri,
                    _propertyName,
                    "/",
                    _item.name,
                    ipfsData[_item.referenceSlot].extension
                )
            )
        );
    }
    ...
    function contractURI() external view override returns (string memory) {
        ...
        items[1] =
            MetadataBuilder.JSONItem({key: MetadataJSONKeys.keyDescription, value: settings.description, quote: true});
        items[2] =
            MetadataBuilder.JSONItem({key: MetadataJSONKeys.keyImage, value: settings.contractImage, quote: true});
